...
DB_PATH="/usr/local/PhantomDave/filebrowser.db"
DEFAULT_PORT=8080
...
if [[ "${install_prompt,,}" =~ ^(y|yes)$ ]]; then
    ...
    chown root:root /usr/local/PhantomDave
    chmod 755 /usr/local/PhantomDave
    touch "$DB_PATH"
...
[Service]
User=root
WorkingDirectory=/usr/local/PhantomDave
ExecStartPre=/bin/touch /usr/local/PhantomDave/filebrowser.db
ExecStartPre=/usr/local/bin/filebrowser config set -a "0.0.0.0" -p ${PORT} -d /usr/local/PhantomDave/filebrowser.db
ExecStart=/usr/local/bin/filebrowser -r / -d /usr/local/PhantomDave/filebrowser.db -p ${PORT}
Restart=always
...
directory="/usr/local/PhantomDave"
